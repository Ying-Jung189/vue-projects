...
const handleSubmit = async () => {
  if (isEdit.value) {
      usersStore.editUser(formData.value.id, formData.value);
    } else {
...
<template>
  <el-card>
    <div v-if="usersStore.loading">載入中...</div>
    <div v-else-if="usersStore.error">{{ error }}</div>
    <div class="upper-nav">
      <el-button type="primary" @click="openAdd"> + 新增員工 </el-button>
    </div>
    <el-table class="employee-list" :data="usersStore.users" style="width: 100%">
